...
export const createHolidayService = async (data: CreateHolidayInput) => {
...
export const getHolidaysService = async (year?: number, month?: number) => {
  ...
  return findAllHolidays(startDate, endDate);
};

export const getHolidayService = async (id: string) => {
...
export const deleteHolidayService = async (id: string, actorId?: string) => {
  if (!Types.ObjectId.isValid(id)) {
    throw new AppError("Invalid holiday ID", 400, "INVALID_HOLIDAY_ID");
  }
