...
export const findLeaveRequestById = async (
  ...
  return LeaveRequest.findById(id)
    .populate("employeeId", "employeeCode name email managerId")
 ...
 */
export const findLeaveRequestByIdForUpdate = async (
  id: string,
  session?: ClientSession,
): Promise<ILeaveRequest | null> => {
...
export const findEmployeeLeaveRequests = async (
  employeeId: string,
): Promise<ILeaveRequest[]> => {
...
export const findPendingLeaveRequests = async (): Promise<ILeaveRequest[]> => {
  return LeaveRequest.find({
    status: "PENDING",
  })
    .populate("employeeId", "employeeCode name email managerId departmentId")
    .populate("leaveTypeId", "name code")
    .sort({
      createdAt: 1,
    });
...
export const updateLeaveRequest = async (
  ...
  const query = LeaveRequest.findByIdAndUpdate(id, data, {
    new: true,
    runValidators: true,
  });
